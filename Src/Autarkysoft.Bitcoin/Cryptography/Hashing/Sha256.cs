...
namespace Autarkysoft.Bitcoin.Cryptography.Hashing
{
    ...
    public sealed class Sha256 : IDisposable
    {
        ...
        public byte[] ComputeTaggedHash(string tag, out bool usedOptimization, params byte[][] data)
        {
            ...
            switch (tag)
            {
                ...
                case "TapTweak":
                    if (data.Length == 1)
                    {
                        return ComputeTaggedHash_TapTweak(data[0]);
                    }
                    else if (data.Length == 2)
                    {
                        return ComputeTaggedHash_TapTweak(data[0], data[1]);
                    }
                    else
                    {
        ...
        internal unsafe byte[] ComputeTaggedHash_TapTweak(ReadOnlySpan<byte> key)
        {
            Debug.Assert(key != null && key.Length == 32);

            // Total data length to be hashed is 96 ([32+32] + 32)
            fixed (byte* pt1 = &key[0])
            fixed (uint* hPt = &hashState[0], wPt = &w[0])
            {
                // The first 64 bytes (1 block) is equal to SHA256("TapTweak") | SHA256("TapTweak")
                // This can be pre-computed and change the HashState's initial value
                hPt[0] = 0xd129a2f3;
                hPt[1] = 0x701c655d;
                hPt[2] = 0x6583b6c3;
        ...
        internal unsafe byte[] ComputeTaggedHash_TapTweak(ReadOnlySpan<byte> key, ReadOnlySpan<byte> merkleRoot)
        {
            Debug.Assert(key != null && key.Length == 32);
            Debug.Assert(merkleRoot != null && merkleRoot.Length == 32);

            // Total data length to be hashed is 128 ([32+32] + 32 + 32)
            fixed (byte* pt1 = &key[0], pt2 = &merkleRoot[0])
            fixed (uint* hPt = &hashState[0], wPt = &w[0])
            {
                // The first 64 bytes (1 block) is equal to SHA256("TapTweak") | SHA256("TapTweak")
                // This can be pre-computed and change the HashState's initial value
