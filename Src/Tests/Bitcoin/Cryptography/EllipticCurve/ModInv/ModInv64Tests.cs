...
namespace Tests.Bitcoin.Cryptography.EllipticCurve.ModInv
{
    public class ModInv64Tests
    {
        ...
        internal static Span<ushort> MulMod256(ReadOnlySpan<ushort> a, ushort[]? b, ushort[]? m)
        {
        ...
        private static ModInv64Signed62 UShortToSigned62(ReadOnlySpan<ushort> arr)
        {
            long[] v = new long[5];
            for (int i = 0; i < 256; i++)
            {
                v[i / 62] |= (long)((arr[i >> 4] >> (i & 15)) & 1) << (i % 62);
            }
            return new ModInv64Signed62(v);
        }
        ...
        private static ushort[] Signed62ToUint16(in ModInv64Signed62 x)
        {
            ushort[] result = new ushort[16];
            long[] xv = x.GetArray();
            for (int i = 0; i < 256; i++)
            {
                result[i >> 4] |= (ushort)(((xv[i / 62] >> (i % 62)) & 1) << (i & 15));
            }
            return result;
        }
        ...
        private static ModInv64Signed62 MutateSignSigned62(in ModInv64Signed62 x, TestRNG rng)
        {
            ...
            for (int i = 0; i < 8; i++)
            {
                int pos = (int)rng.RandBits(2);
                if (xv[pos] > 0 && xv[pos + 1] <= M62)
                {
                    xv[pos] -= M62 + 1;
                    xv[pos + 1] += 1;
                }
                else if (xv[pos] < 0 && xv[pos + 1] >= -M62)
                {
                    xv[pos] += M62 + 1;
                    xv[pos + 1] -= 1;
                }
            ...
            return new ModInv64Signed62(xv);
        }

        // test if a and b are coprime
        internal static bool Coprime(ReadOnlySpan<ushort> a, ReadOnlySpan<ushort> b)
        {
            ...
            while (true)
            {
                ...
                Span<ushort> t = MulMod256(y, null, x.ToArray());
                x.CopyTo(y);
                t.CopyTo(x);
            }
        ...
        /// run_modinv_tests
        /// </summary>
        public static IEnumerable<TheoryDataRow<ushort[], ushort[], ushort[]?, TestRNG>> GetCases()
        {
            TestRNG rng = new();
            ...
            foreach (var item in Cases)
            {
                ...
                yield return new(item[0], item[1], item[2], rng);
            }
            ...
            for (int i = 0; i < 100 * COUNT; i++)
            {
                ...
                yield return new(xd, md, null, rng);

                // In a few cases, also test with input=0
                if (i < COUNT)
                {
                    yield return new(ZERO, md, null, rng);
                }
            }
        }
        ...
        public void Libsecp256k1_ModInv64_UInt16Test(ushort[] input, ushort[] mod, ushort[]? expected, TestRNG rng)
        {
            const long M62 = (long)(ulong.MaxValue >> 2);

            ModInv64Signed62 x = UShortToSigned62(input);
            bool nonzero = (x.v0 | x.v1 | x.v2 | x.v3 | x.v4) != 0;
            ModInv64Signed62 modulus = UShortToSigned62(mod);

            // compute 1/modulus mod 2^30
            ulong modinv62 = ModInv2p64((ulong)modulus.v0) & M62;
            ModInv64ModInfo m = new(modulus, modinv62);
            Assert.Equal(1UL, (m.modulus_inv62 * (ulong)m.modulus.v0) & M62);
            ...
            x = UShortToSigned62(input);
            m = new ModInv64ModInfo(MutateSignSigned62(m.modulus, rng), m.modulus_inv62);
            ...
            for (int vartime = 0; vartime < 2; vartime++)
            {
                ...
                {
                    // Constant Time
                    ModInv64.Compute(ref x, m);
                }
                else
                {
                    // Variable Time
                    ModInv64.ComputeVar(ref x, m);
                }

                // produce output
                ushort[] actual = Signed62ToUint16(x);
                ...
                if (expected != null)
                {
                    // These are fixed test cases (our static field)
        ...
        internal static readonly ushort[][][] Cases =
        [
