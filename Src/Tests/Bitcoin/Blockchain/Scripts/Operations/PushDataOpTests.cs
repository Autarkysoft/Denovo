...
namespace Tests.Bitcoin.Blockchain.Scripts.Operations
{
    public class PushDataOpTests
    {
        ...
        {
            byte[] data = { 1, 2, 3 };
            MockSerializableScript scr = new(data, 255);
            PushDataOp op = new(scr);
        ...
        {
            PushDataOp result = new();
            Assert.True(result.TryRead(new FastStreamReader(BadEncoding), out Errors error), error.Convert());
            return result;
        }

        public static IEnumerable<object[]> GetRunCases()
        {
            yield return new object[] { new PushDataOp(0L), Array.Empty<byte>() };
        ...
        {
            MockOpData opData = new(FuncCallName.Push)
            {
                _itemCount = 0,
                _altItemCount = 0,
        ...
        {
            PushDataOp op = new(OpTestCaseHelper.b1);
            MockOpData opData = new(FuncCallName.Push)
            {
                _itemCount = 1001,
                _altItemCount = 0,
        ...
        public void Run_ItemLenghOverflowTest()
        {
            ...
            bool b1 = op.TryRead(new FastStreamReader(data), out Errors error);
        ...
        {
            PushDataOp op = new();
            FastStreamReader stream = new(new byte[2] { 1, 2 });
            bool didRead = op.TryRead(stream, out Errors error);

            Assert.True(didRead, error.Convert());
            Assert.Equal(Errors.None, error);
        ...
        public static IEnumerable<object[]> GetReadCases()
        {
            ...
            yield return new object[] { new byte[1], null, OP._0 };
            yield return new object[] { new byte[] { 0x4f }, null, OP.Negative1 };
            yield return new object[] { new byte[] { 0x51, 2 }, null, OP._1 };
            yield return new object[] { new byte[] { 0x60 }, null, OP._16 };

            yield return new object[] { new byte[] { 1, 0 }, new byte[1], (OP)1 }; // not strict
            yield return new object[] { new byte[] { 1, 3, 4 }, new byte[1] { 3 }, (OP)1 }; // not strict

            yield return new object[] { data77, exp77, OP.PushData1 };

            yield return new object[] { BadEncoding, BadEncodingData, OP.PushData1 };
        }
        [Theory]
        [MemberData(nameof(GetReadCases))]
        public void TryReadTest(byte[] data, byte[] expData, OP expOP)
        {
            PushDataOp op = new();
            FastStreamReader stream = new(data);
            bool b = op.TryRead(stream, out Errors error);
        ...
        {
            PushDataOp op = new();
            // PubkeyScript of Tx ID on TestNet3: 88bac1e84c235aa0418345bf430fb43b336875974b6e87dc958de196f9222c35
            ...
            FastStreamReader stream = new(Helper.HexToBytes($"{dataLenHex}{dataHex}"));
            bool b = op.TryRead(stream, out Errors error);
        ...
        public void TryRead_FailTest(FastStreamReader stream, Errors expErr)
        {
            PushDataOp op = new();
            bool b = op.TryRead(stream, out Errors error);
        ...
        {
            FastStream normalStream = new();
            FastStream witnessStream = new();
            FastStream witnessSignStream = new();
        ...
        public static IEnumerable<object[]> WriteToStreamSigningSingleCases()
        {
            ...
            yield return new object[] { new PushDataOp(new byte[] { 10, 20, 30 }), sig, Array.Empty<byte>() };
        ...
        public static IEnumerable<object[]> WriteToStreamSigningMultiCases()
        {
            ...
            // Remove signature
            yield return new object[] { new PushDataOp(new byte[] { 10, 20, 30 }), sigs, Array.Empty<byte>() };
            yield return new object[] { new PushDataOp(new byte[] { 40, 50, 60, 70 }), sigs, Array.Empty<byte>() };
            yield return new object[] { new PushDataOp(new byte[] { 110, 111, 112, 113, 114, 115 }), sigs, Array.Empty<byte>() };

            // Bad encoding with equal signature which is not removed
            PushDataOp badPush1 = new();
            byte[] badBytes1 = new byte[] { (byte)OP.PushData1, 3, 10, 20, 30 };
            Assert.True(badPush1.TryRead(new FastStreamReader(badBytes1), out Errors er), er.Convert());
            yield return new object[] { badPush1, sigs, badBytes1 };

            // Bad encoding with unequal signature which is not removed
            PushDataOp badPush2 = new();
            byte[] badBytes2 = new byte[] { (byte)OP.PushData1, 5, 10, 20, 30, 40, 50 };
            Assert.True(badPush2.TryRead(new FastStreamReader(badBytes2), out er), er.Convert());
